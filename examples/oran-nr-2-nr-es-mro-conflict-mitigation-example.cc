 ...
 */
/**
 * NR (5G) counterpart of oran-lte-2-lte-es-mro-conflict-mitigation-example.cc.
 *
 * Exercises: OranLmNr2NrRsrpHandover, OranLmNr2NrEnergySaving,
 *            OranCmmNr2NrEsMro, OranCommandNr2NrTxPower,
...
NS_LOG_COMPONENT_DEFINE("OranNr2NrEsMroConflictMitigationExample");
...
main(int argc, char* argv[])
{
    ...
    std::string dbFileName = "oran-nr-es-mro-conflict-repository.db";
    ...
    LogComponentEnable("OranCmmNr2NrEsMro", (LogLevel)(LOG_PREFIX_TIME | LOG_INFO));
    ...
    for (uint32_t idx = 0; idx < ueNodes.GetN(); idx++)
    {
        ...
        Ptr<NrUeNetDevice> nrUeDevice = DynamicCast<NrUeNetDevice>(ueNetDev.Get(idx));
        if (nrUeDevice)
        {
