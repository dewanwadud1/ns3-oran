 ...
 */
/**
 * DQN Energy-Saving Logic Module for NR, using an ONNX model for inference.
 * See oran-lm-nr-2-nr-onnx-energy-saving.h for the state/action layout,
 * and workspace/ml/es_dqn/train_es_dqn.py for how the model was trained.
 */

#include "oran-lm-nr-2-nr-onnx-energy-saving.h"
...
NS_LOG_COMPONENT_DEFINE("OranLmNr2NrOnnxEnergySaving");
NS_OBJECT_ENSURE_REGISTERED(OranLmNr2NrOnnxEnergySaving);

TypeId
OranLmNr2NrOnnxEnergySaving::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::OranLmNr2NrOnnxEnergySaving")
            .SetParent<OranLm>()
            .AddConstructor<OranLmNr2NrOnnxEnergySaving>()
            .AddAttribute("OnnxModelPath",
                          "The file path of the trained ES DQN ONNX model.",
                          StringValue("es_dqn_nr.onnx"),
                          MakeStringAccessor(&OranLmNr2NrOnnxEnergySaving::SetOnnxModelPath),
                          MakeStringChecker())
            .AddAttribute("NominalTxPowerDbm",
                          "The 'awake' TxPower (dBm); 'asleep' gNBs are dropped to 0 dBm. "
                          "Should match the scenario's initial --tx-power.",
                          DoubleValue(35.0),
                          MakeDoubleAccessor(&OranLmNr2NrOnnxEnergySaving::m_nominalTxPowerDbm),
                          MakeDoubleChecker<double>(0.0, 50.0));

    return tid;
}

OranLmNr2NrOnnxEnergySaving::OranLmNr2NrOnnxEnergySaving()
{
    NS_LOG_FUNCTION(this);

    m_name = "OranLmNr2NrOnnxEnergySaving";
}
...
void
OranLmNr2NrOnnxEnergySaving::SetOnnxModelPath(const std::string& onnxModelPath)
{
    std::ifstream f(onnxModelPath.c_str());
    NS_ABORT_MSG_IF(!f.good(),
                    "ONNX model file \""
                        << onnxModelPath << "\" not found."
                        << " Sample model \"es_dqn_nr.onnx\" can be copied from"
...
std::vector<OranLmNr2NrOnnxEnergySaving::EnbRsrpStats>
OranLmNr2NrOnnxEnergySaving::GetEnbRsrpStats(Ptr<OranDataRepository> data,
                                             const std::vector<uint64_t>& enbIds) const
{
    ...
    // build_state (RMIN_DBM = -95.0), so a gNB with no visible UEs yet
...
OranLmNr2NrOnnxEnergySaving::Run()
{
    NS_LOG_FUNCTION(this);
    ...
    NS_ABORT_MSG_IF(m_nearRtRic == nullptr, "OranLmNr2NrOnnxEnergySaving: no Near-RT RIC");
    ...
    // t/T: training varied this over a bounded episode, but a continuously
    // running LM has no equivalent bounded horizon, so a fixed mid-point
    // placeholder is used here at inference time (see the LTE ONNX ES LM
    ...
    if (!anyAwake)
    {
        // Matches the training-time safeguard: never let every gNB sleep at once.
    ...
    for (std::size_t i = 0; i < numRus; ++i)
    {
        ...
        NS_LOG_INFO("gNB " << enbId << (wantAwake[i] ? ": waking" : ": sleeping") << " (Δ="
