 ...
 */
#include "oran-lm-nr-2-nr-coverage-capacity-optimization.h"
...
NS_LOG_COMPONENT_DEFINE("OranLmNr2NrCoverageCapacityOptimization");
NS_OBJECT_ENSURE_REGISTERED(OranLmNr2NrCoverageCapacityOptimization);
...
TypeId
OranLmNr2NrCoverageCapacityOptimization::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::OranLmNr2NrCoverageCapacityOptimization")
            .SetParent<OranLm>()
            .AddConstructor<OranLmNr2NrCoverageCapacityOptimization>()
            .AddAttribute("LowRsrpThresholdDbm",
                          "Serving-cell RSRP threshold below which CCO raises TxPower.",
                          DoubleValue(-110.0),
                          MakeDoubleAccessor(
                              &OranLmNr2NrCoverageCapacityOptimization::m_lowRsrpThresholdDbm),
                          MakeDoubleChecker<double>())
            .AddAttribute("LowRsrpFractionThreshold",
                          "Fraction of low-RSRP samples required before acting.",
                          DoubleValue(0.10),
                          MakeDoubleAccessor(&OranLmNr2NrCoverageCapacityOptimization::
                                                 m_lowRsrpFractionThreshold),
                          MakeDoubleChecker<double>(0.0, 1.0))
            .AddAttribute("StepSize",
                          "TxPower increase requested by CCO in dB.",
                          DoubleValue(1.0),
                          MakeDoubleAccessor(&OranLmNr2NrCoverageCapacityOptimization::m_stepSizeDb),
                          MakeDoubleChecker<double>(0.0))
            .AddAttribute("MinSamplesPerCell",
                          "Minimum serving-RSRP samples per cell before CCO acts.",
                          UintegerValue(1),
                          MakeUintegerAccessor(
                              &OranLmNr2NrCoverageCapacityOptimization::m_minSamplesPerCell),
            ...
            .AddAttribute("RetStepDeg",
                          ...
                          DoubleValue(1.0),
                          MakeDoubleAccessor(
                              &OranLmNr2NrCoverageCapacityOptimization::m_retStepDeg),
                          MakeDoubleChecker<double>());
    return tid;
}

OranLmNr2NrCoverageCapacityOptimization::OranLmNr2NrCoverageCapacityOptimization()
    : OranLm(),
      ...
      m_retStepDeg(1.0)
{
    m_name = "OranLmNr2NrCoverageCapacityOptimization";
}

OranLmNr2NrCoverageCapacityOptimization::~OranLmNr2NrCoverageCapacityOptimization() = default;

std::vector<Ptr<OranCommand>>
OranLmNr2NrCoverageCapacityOptimization::Run()
{
    ...
    for (auto ueId : data->GetNrUeE2NodeIds())
    {
        ...
        std::tie(found, servingCellId, servingRnti) = data->GetNrUeCellInfo(ueId);
        if (!found)
            continue;
    ...
    for (const auto& item : stats)
    {
        ...
        {
            Ptr<OranCommandNr2NrTxPower> txCmd = CreateObject<OranCommandNr2NrTxPower>();
        ...
        if (critFraction >= m_criticalFractionThreshold && m_retStepDeg != 0.0)
        {
            ...
            newRet = std::max(0.0, std::min(15.0, newRet));

            Ptr<OranCommandNr2NrCellParameter> retCmd =
                CreateObject<OranCommandNr2NrCellParameter>();
