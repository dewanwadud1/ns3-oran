 ...
 */
#include "oran-cmm-nr-2-nr-es-mro.h"
...
NS_LOG_COMPONENT_DEFINE("OranCmmNr2NrEsMro");
NS_OBJECT_ENSURE_REGISTERED(OranCmmNr2NrEsMro);
    ...
    std::map<std::tuple<std::string, bool>, std::vector<Ptr<OranCommand>>> inputCommands)
{
    ...
    for (const auto& commandSet : inputCommands)
    {
        ...
        {
            Ptr<OranCommandNr2NrHandover> handover =
            ...
            Ptr<OranCommandNr2NrTxPower> txPower =
            ...
            Ptr<OranCommandNr2NrTxPower> mitigated =
...
bool
OranCmmNr2NrEsMro::ShouldForwardHandover(Ptr<OranCommand> command)
{
    if (!m_active)
    {
        return true;
    ...
    if (!found)
    {
        LogLogicToStorage("Dropping handover command because serving gNB cell info is unknown");
        return false;
    }

    const uint64_t ueE2NodeId =
        m_nearRtRic->Data()->GetNrUeE2NodeIdFromCellInfo(servingCellId,
...
double
OranCmmNr2NrEsMro::SelectFraction(double requestedDeltaDb) const
{
...
double
OranCmmNr2NrEsMro::SolveGame(const std::string& method) const
{
